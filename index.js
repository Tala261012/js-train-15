...
function generateRandomPassword(length) {
  ...
  do {
    ...
    if (i < characters.length) pass += characters.at(i);
...
function getFractionalPart(num) {
  ...
  let roundNum = Math.trunc(num);

  return Math.fround(num - roundNum);
}
...
function estimateInvestment(principal, interestRate, years) {
  ...
  if (
    ...
    console.log("Помилка: всi аргументи мають бути числами.");
    return null;
  }

  return Math.round(principal * (interestRate + 1) ** years);
}
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  if (!Array.isArray(products) || typeof maxPrice !== "number") {
    console.log("Помилка: аргументи мають бути масивом та числом.");
    return null;
  }

  let totalPrice = products.reduce((sum, obj) => (sum += obj.price), 0);
